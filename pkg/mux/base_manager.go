...
type ManagerHooks interface {
	...
	Launch(ctx context.Context, cfg *sqlc.Runner, token string) error
	...
	Cleanup(runner sqlc.Runner, token string) error
...
func (b *BaseManager) Start(ctx context.Context, cfg sqlc.Runner, token string) error {
	b.Mu.Lock()
	...
	if err := b.Hooks.Launch(ctx, &cfg, token); err != nil {
		b.Mu.Lock()
