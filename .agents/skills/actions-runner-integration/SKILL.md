...
- **Cleanup Mechanism:** The worker container's `_work` directory is generated strictly inside the container's ephemeral filesystem layer at `/actions-runner/_work` (relative to `cmd.Dir = "/actions-runner"` in the worker-launcher). Ephemeral cleanup is entirely dependent on Docker auto-removing the container (`AutoRemove: true`). Do not implement manual host-side cleanup scripts for `_work`.
...
- **Runner Dir Resolution:** The shim derives the runner directory from its own executable path (`filepath.Dir(filepath.Dir(os.Executable()))`), since the shim is installed at `<cfg.Dir>/bin/Runner.Worker`. The shim uses this path to directly read the config files from its local disk (`cmd/worker-shim/config.go`) and package them into the TCP header. The Orchestrator is completely decoupled from this process and no longer returns config files in the allocation API.
