...
The official GitHub Actions runner forces you to run a 1:1 mapping of runner processes to repositories or organizations. This multiplexer and orchestrator changes the paradigm: it acts as a central proxy that listens to an infinite number of scopes simultaneously, and dynamically provisions perfectly isolated, ephemeral Docker containers the exact moment a job arrives.
...
- **Simple Authentication:** No need to manage complex GitHub App credentials; authenticate easily using standard GitHub Registration Tokens.
...
1. **The Multiplexer**: A single Go process that boots and manages multiple lightweight `Runner.Listener` processes.
